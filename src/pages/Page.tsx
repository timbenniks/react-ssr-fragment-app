  ...
  if (!content) {
    return (
      <main className="max-w-4xl mx-auto px-4 py-8">
        <section className="bg-amber-50 border border-amber-200 rounded-lg p-6" role="alert">
          <h1 className="text-2xl font-bold text-amber-800 mb-2">Page Not Found</h1>
          <p className="text-amber-700">
            No content found for: <code className="bg-amber-100 px-2 py-1 rounded font-mono text-sm">{pathname}</code>
          </p>
        </section>
      </main>
    );
      ...
      {content.blocks?.length ? (
        <section className="space-y-8">
          {content.blocks.map((wrapper) => (
            <BlockComponent key={wrapper.block._metadata?.uid || wrapper.uid} block={wrapper.block} />
          ))}
        </section>
      ) : null}
    </main>
